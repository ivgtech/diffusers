...
def get_parquet_files_list(url):
    ...
    if response.status_code == 200:
        ...
        raise Exception(
            f"Failed to retrieve Parquet files. Status code: {response.status_code}"
        )
...
def main():
    ...
    ):  # Ensure the last worker picks up any remaining files
    ...
    print(
        f"SCRIPT.PY: {end_index - start_index} URLs assigned for download by worker {worker_id}."
    )
