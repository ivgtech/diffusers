...
pipeline, pipeline_params = FlaxStableDiffusionInstructPix2PixPipeline.from_pretrained(
    ...
    "./april30-instruct-pix2pix-model",
    dtype=jnp.bfloat16,
    safety_checker=None,
)
...
if LOAD_NON_EMA_PARAMS:
    ...
    with open(params_path, "rb") as f:
        non_ema_params = from_bytes(
            pipeline_params["unet"], f.read()
        )  # template object (here params['unet']) needs to be isomorphic to the target object
...
# HF Diffusers train_instruct_pix2pix.py defaults: 20, 10, 1.5
hf_num_inference_steps = 20
hf_image_guidance_scale = 1.5
hf_guidance_scale = 10

# Defaults used by author's: 100, 7.5, 1.2
instructions_pix2pix_seed = 1371
instruct_pix2pix_num_inference_steps = 100
instruct_pix2pix_guidance_scale = 7.5
instruct_pix2pix_image_guidance_scale = 1.2

# HF FlaxStableDiffusionPipeline defaults: 50, 7.5, no image guidance
...
print(
    f"n-steps: {NUM_INFERENCE_STEPS}, guidance_scale: {GUIDANCE_SCALE}, image_guidance_scale: {IMAGE_GUIDANCE_SCALE}"
)
